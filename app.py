...
# ================= NOTIFS (inject + refresh) =================

@app.context_processor
def inject_notifs():
    # Valeur par défaut pour les templates
...
def refresh_notifs_count():
    ...
    if session.get("role") == "Admin":
        try:
            ...
            # si RLS / table / colonnes / cache schema -> on évite de planter l'app
            session["nb_notifs"] = 0
    else:
        session["nb_notifs"] = 0
...
def accueil():
    ...
    # Le compteur notifs est déjà rafraîchi par before_request
...
def action_materiel():
    ...
    # REFORME (retire une quantité du stock de la ligne)
    if action == "reforme":
        supabase.table("materiels").update({
            "quantite": stock - qte
        }).eq("id", mid).execute()
...
def admin_demande_echange():
    ...
    # notification ADMIN (SANS colonne "type")
